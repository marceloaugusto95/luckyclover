...
// Fetch recent contests from the API (returns array)
export async function getRecentMegaSenaResults(
    limit = 20,
    ...
    try {
        ...
        // API returns array of contests in descending order
        const results: MegaSenaResult[] = data.slice(0, limit).map((
            item: any,
        ) => ({
            ...
            proximoConcurso: item.proximoConcurso || item.concurso + 1,
            dataProximoConcurso: item.dataProximoConcurso || "A definir",
        }));

        return results;
    } catch (err) {
        console.warn("Mega Sena API failed:", err);
        return [];
    }
}

// Fetch a specific contest by number
export async function getMegaSenaResultByNumber(
    concurso: number,
): Promise<MegaSenaResult | null> {
    try {
        const controller = new AbortController();
        ...
        return {
            ...
            proximoConcurso: data.proximoConcurso || data.concurso + 1,
            dataProximoConcurso: data.dataProximoConcurso || "A definir",
        };
    } catch (err) {
        console.warn(`Failed to fetch contest ${concurso}:`, err);
        return null;
    }
}

// Fetch the latest contest — DB-first approach
