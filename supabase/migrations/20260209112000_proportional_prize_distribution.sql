-- Migration: Implement Proportional Prize Distribution
-- Purpose: Ensure Sena and Quina prizes are always proportional (70/30) relative to the TOTAL pool (current + accumulations).
...
DECLARE
    ...
    v_current_pool DECIMAL(12,2) := 0;
    v_grand_pool DECIMAL(12,2) := 0;
...
BEGIN
    ...
    v_current_pool := v_total_sales * 0.70;
    ...
    -- Grand Pool = Current + All previous accumulations
    v_grand_pool := v_current_pool + v_prev_accumulated_sena + v_prev_accumulated_quina;
    
    -- Calculate Final Prize Portions based on proportional split (70/30 by default)
    v_final_sena_prize := v_grand_pool * v_sena_pct;
    v_final_quina_prize := v_grand_pool * v_quina_pct;
    
    -- Process all PAID bets
    FOR v_bet IN 
        SELECT id, numbers
        FROM public.bets
    ...
    SET 
        ...
        -- Rollover logic: If no winners in a category, that category's portion rolls over.
        -- BUT, since the pool is recalculated proportionally next time, we just need to store 
        -- what didn't get paid out.
...
BEGIN
    ...
    -- Current Pool = 70% of Total Revenue
    current_pool := total_sales * 0.70;
    ...
    -- Grand Pool for estimation
    grand_pool := current_pool + prev_accumulated_sena + prev_accumulated_quina;

    -- Calculate prizes proportionally
    sena_prize := grand_pool * sena_pct;
    quina_prize := grand_pool * quina_pct;

    RETURN json_build_object(
        'total_sales', total_sales,
        'prize_pool', grand_pool, -- Represent total available money
        'sena', sena_prize,
        'quina', quina_prize,
        'quadra', 0, 
